                                        ...
                                        <tr>
                                            ...
                                            {{-- <td>
                                                <div class="btn-group" role="group" aria-label="Basic example">
                                                    <a href="{{ route('edit.roles.permission', $row->id)  }}" span class="btn btn-success btn-sm mr-2" style="margin-right: 4px;"><i class="fas fa-edit"></i></a>
                                                    <a href="{{ route('admin.roles.delete', $row->id)  }}" span class="btn btn-danger btn-sm mr-2" style="margin-right: 4px;"><i class="fas fa-trash"></i></a>
                                                </div>
                                            </td> --}}
                                        </tr>
                                    @endforeach
                            ...
                            <form action="{{ route('permission.store') }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div data-tw-merge class="p-5 grid grid-cols-12 gap-4 gap-y-3">
                                    <div class="col-span-12 sm:col-span-12">
                                        ...
                                        <input data-tw-merge id="modal-form-1" type="text" name="name" placeholder="example@gmail.com" class="disabled:bg-slate-100 disabled:cursor-not-allowed dark:disabled:bg-darkmode-800/50 dark:disabled:border-transparent [&[readonly]]:bg-slate-100 [&[readonly]]:cursor-not-allowed [&[readonly]]:dark:bg-darkmode-800/50 [&[readonly]]:dark:border-transparent transition duration-200 ease-in-out w-full text-sm border-slate-200 shadow-sm rounded-md placeholder:text-slate-400/90 focus:ring-4 focus:ring-primary focus:ring-opacity-20 focus:border-primary focus:border-opacity-40 dark:bg-darkmode-800 dark:border-transparent dark:focus:ring-slate-700 dark:focus:ring-opacity-50 dark:placeholder:text-slate-500/80 [&[type='file']]:border file:mr-4 file:py-2 file:px-4 file:rounded-l-md file:border-0 file:border-r-[1px] file:border-slate-100/10 file:text-sm file:font-semibold file:bg-slate-100 file:text-slate-500/70 hover:file:bg-200 group-[.form-inline]:flex-1 group-[.input-group]:rounded-none group-[.input-group]:[&:not(:first-child)]:border-l-transparent group-[.input-group]:first:rounded-l group-[.input-group]:last:rounded-r group-[.input-group]:z-10">
                                    </div>
                                    <div class="col-span-12 sm:col-span-12">
                                        ...
                                        <select data-tw-merge id="modal-form-6" name="group_name" class="disabled:bg-slate-100 disabled:cursor-not-allowed disabled:dark:bg-darkmode-800/50 [&[readonly]]:bg-slate-100 [&[readonly]]:cursor-not-allowed [&[readonly]]:dark:bg-darkmode-800/50 transition duration-200 ease-in-out w-full text-sm border-slate-200 shadow-sm rounded-md py-2 px-3 pr-8 focus:ring-4 focus:ring-primary focus:ring-opacity-20 focus:border-primary focus:border-opacity-40 dark:bg-darkmode-800 dark:border-transparent dark:focus:ring-slate-700 dark:focus:ring-opacity-50 group-[.form-inline]:flex-1">
                                            <option selected>Select Option ..</option>
                                            <option value="Dashboard">Dashboard</option>
                                            <option value="Stock">Stock</option>
                                            <option value="Prospecting Beli">Prospecting Beli</option>
                                            <option value="Prospecting Jual">Prospecting Jual</option>
                                            <option value="Lead Sales">Lead Sales</option>
                        ...
                        <div class="modal-content w-[90%] mx-auto bg-white relative rounded-md shadow-md transition-[margin-top,transform] duration-[0.4s,0.3s] mt-16 dark:bg-darkmode-600 sm:w-[460px]">
                            ...
                            <form id="edit-form" action="" method="POST">
                                ...
                                <div class="p-5 grid grid-cols-12 gap-4 gap-y-3">
                                    <div class="col-span-12 sm:col-span-12">
                                        <label for="edit-name" class="inline-block mb-2">Name</label>
                                        <input id="edit-name" name="name" type="text" class="w-full text-sm border-slate-200 shadow-sm rounded-md">
                                    </div>
                                    <div class="col-span-12 sm:col-span-12">
                                        <label data-tw-merge for="modal-form-6" class="inline-block mb-2 group-[.form-inline]:mb-2 group-[.form-inline]:sm:mb-0 group-[.form-inline]:sm:mr-5 group-[.form-inline]:sm:text-right">
                                            Nama Group
                                        </label>
                                        <select data-tw-merge id="edit-group" name="group_name" class="disabled:bg-slate-100 disabled:cursor-not-allowed disabled:dark:bg-darkmode-800/50 [&[readonly]]:bg-slate-100 [&[readonly]]:cursor-not-allowed [&[readonly]]:dark:bg-darkmode-800/50 transition duration-200 ease-in-out w-full text-sm border-slate-200 shadow-sm rounded-md py-2 px-3 pr-8 focus:ring-4 focus:ring-primary focus:ring-opacity-20 focus:border-primary focus:border-opacity-40 dark:bg-darkmode-800 dark:border-transparent dark:focus:ring-slate-700 dark:focus:ring-opacity-50 group-[.form-inline]:flex-1">
        ...
        $(document).ready(function() {
            var table = $('#inventory-table').DataTable({
                ...
                columns: [
                    ...
                    { data: 'group_name', name: 'group_name', searchable: true , className: 'text-center' ,
                                    ...
                                    <div class="dropdown relative h-5">
                                        ...
                                        <div class="dropdown-menu absolute z-[9999] hidden">
                                            <div class="dropdown-content rounded-md border-transparent bg-white p-2 shadow-[0px_3px_10px_#00000017] dark:border-transparent dark:bg-darkmode-600 w-40">
                                                <a href="#" onclick="openEditModal(${row.id}, '${row.name}', '${row.group_name ?? ''}')" class="cursor-pointer flex items-center p-2 transition duration-300 ease-in-out rounded-md hover:bg-slate-200/60 dark:bg-darkmode-600 dark:hover:bg-darkmode-400 dropdown-item">
...
<script>
    function openEditModal(id, name, group_name) {

    // Populate the form fields with data
    document.getElementById('edit-name').value = name;

    // Reset the group_name field
    document.getElementById('edit-group').value = '';

    // If group_name is not empty, select the corresponding option
    if (group_name) {
        document.getElementById('edit-group').value = group_name;
    }
    ...
    function closeEditModal() {
        ...
        // Reload the page after a short delay to ensure the modal is hidden first
        // setTimeout(() => {
        //     window.location.reload();
        // }, 200); // 200ms delay to allow the modal to fully hide before reloading
    }
</script>
@endpush
